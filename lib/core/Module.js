...
function loadModules(next) {
  ...
  /* Make a list of modules to load from different configuration sources. */
  var modulePaths=[];

      // Load from server:modulePath, ./{type}/{module}/module.js.
      function fromModuleBasePath(moduleBasePath, pathsList) {
               ...
               if (type != "README" && type[0] != '.') { // Ignore the readme file and .DS_Store file for Macs
                   calipso.lib.fs.readdirSync(path.join(moduleBasePath, type)).forEach(function(moduleFolderName) {
                        ...
                        if (moduleFolderName != "README" && moduleFolderName[0] != '.') { // Ignore the readme file and .DS_Store file for Macs
                             ...
                             pathsList.push({name: moduleFolderName, path: modulePath, type: type}); 
                        }
                   });
              }
           });
  ...
  fromModuleBasePath(moduleBasePath, modulePaths);

  // Support for adding explicit module paths in the configuration, as an array. 
  // Always relative to the current working directory.
  var pathsList = calipso.config.get('server:modulePathsList');
  if ( pathsList) {
      var rootpath = process.cwd() + '/';
      pathsList.forEach(function(moduleInfo) { 
             moduleInfo.path ? moduleInfo.path : "./modules/downloaded";
             // add rootpath only if the path is not absolute.
             if ( ! moduleInfo.path.match(/^\//) ) {
                  moduleInfo.path=path.join(rootpath, moduleInfo.path);
             }
             modulePaths.push(moduleInfo); 
      }); 

  }

  modulePaths.forEach(function(moduleInfo) {
      loadModule(moduleInfo.path, moduleInfo.name, moduleInfo.type);
  });
...
/*
 * load a module at specified path.
 */
function loadModule(modulePath, moduleName, type, folderName, libraryName)
{ 
   var module = {
     name: moduleName,
     folder: folderName || moduleName,
     library: libraryName || moduleName,
     type: type,
     path: modulePath,
     enabled: false,
     inited: false
   ...
   module.name = (module.about && module.about.name) ? module.about.name : moduleName;
...
function requireModule(module, modulePath, reload, next) {
  ...
  try {
    ...
    calipso.silly("Loaded module... " + module.name);
    ...
    if (typeof module.fn.routes === "function") {
